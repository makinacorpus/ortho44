...
lieux_json = open("BDAdresse_IGN/lieux-dits.geojson")
...
for line in sdis_json:
	...
	try:
		...
		continue
	out.write("""{ "index" : { "_index" : "cg44", "_type" : "address", "_id" : "%d" } }\n""" % count)
	nom = data['properties'].get('nom', None)
	if not nom:
		continue
	prop['nom'] = nom
	prop['commune'] = data['properties']['COMMUNE']
	prop['type'] = 'LIEUDIT'
	prop['origin'] = 'SDIS44'
	prop['geometry'] = data['geometry']
	out.write(json.dumps(prop)+"\n")
